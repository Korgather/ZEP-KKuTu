...
var MODE;
var BEAT = [null, "10000000", "10001000", "10010010", "10011010", "11011010", "11011110", "11011111", "11111111"];
var NULL_USER = {
...
$(document).ready(function () {
	...
	for (i = 0; i <= 10; i++) $data._soundList.push({ key: "T" + i, value: "/media/kkutu/T" + i + ".mp3" }, { key: "K" + i, value: "/media/kkutu/K" + i + ".mp3" }, { key: "As" + i, value: "/media/kkutu/As" + i + ".mp3" });
	...
	$stage.chatBtn
		.on("click", function (e) {
			...
			var value = mobile && $stage.game.here.is(":visible") ? $stage.game.hereText.val() : $stage.talk.val();
			if (!value) return;
	...
	$(".result-me-gauge").append($("<div>").addClass("graph-bar result-me-current-bar")).append($("<div>").addClass("graph-bar result-me-bonus-bar"));
	// 메뉴 버튼
	...
	$stage.dialog.resultSave.on("click", function (e) {
		...
		var fileName = "KKuTu" + (date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate() + " " + date.getHours() + "-" + date.getMinutes() + "-" + date.getSeconds()) + ".kkt";
		var $a = $("<a>")
			.attr({
				download: fileName,
				href: url,
		...
		reader.onload = function (e) {
			...
			try {
				...
				for (i in data.players) {
					...
					$players.append(($p = $("<div>").addClass("replay-player-bar ellipse").html(u.title).prepend(getLevelImage(u.data.score).addClass("users-level"))));
...
$lib.Classic.turnGoing = function () {
	...
	$stage.game.turnBar.width($data._timePercent()).html(($data._turnTime * 0.001).toFixed(1) + L["SECOND"]);
	$stage.game.roundBar.width(($data._roundTime / $data.room.time) * 0.1 + "%").html(($data._roundTime * 0.001).toFixed(1) + L["SECOND"]);
...
$lib.Jaqwi.roundReady = function (data) {
	...
	$(".jjo-turn-time .graph-bar").width("100%").html(tv).css("text-align", "center");
	drawRound(data.round);
	playSound("round_start");
	clearInterval($data._tTime);
};
...
function checkAge() {
	...
	while (true) {
		...
		if (lv == 4) {
			if (confirm(L["checkAgeSure"] + "\n" + input[0] + L["YEAR"] + " " + input[1] + L["MONTH"] + " " + input[2] + L["DATE"])) return send("caj", { answer: "yes", input: [input[1], input[2], input[0]] }, true);
		} else {
...
function onMessage(data) {
	...
	switch (data.type) {
		case "recaptcha":
			...
			$introText.empty();
			$introText.html("게스트는 캡챠 인증이 필요합니다." + "<br/>로그인을 하시면 캡챠 인증을 건너뛰실 수 있습니다." + "<br/><br/>");
		...
		case "friend":
			...
			if ($data._friends[data.id] && $data.friends[data.id]) notice((data.stat == "on" ? "&lt;<b>" + L["server_" + $data._friends[data.id].server] + "</b>&gt; " : "") + L["friend"] + " " + $data.friends[data.id] + L["fstat_" + data.stat]);
			updateCommunity();
			break;
		case "friendAdd":
		...
		case "error":
			...
			if (data.code == 401) {
				...
				var block = "\n제한 시점: " + blockedUntil.getFullYear() + "년 " + blockedUntil.getMonth() + 1 + "월 " + blockedUntil.getDate() + "일 " + blockedUntil.getHours() + "시 " + blockedUntil.getMinutes() + "분까지";
...
function updateUserList(refresh) {
	...
	$stage.lobby.userListTitle.html("<i class='fa fa-users'></i>" + "&lt;<b>" + L["server_" + $data.server] + "</b>&gt; " + L["UserList"].replace("FA{users}", "") + " [" + len + L["MN"] + "]");
...
function roomListBar(o) {
	...
	$R = $("<div>")
		...
		.append(
			$("<div>")
				.addClass("rooms-lock")
				.html(o.password ? "<i class='fa fa-lock'></i>" : "<i class='fa fa-unlock'></i>")
		)
		.on("click", function (e) {
...
function drawScore($obj, score) {
	var i,
		sc = score > 99999 ? zeroPadding(Math.round(score * 0.001), 4) + "k" : zeroPadding(score, 5);
	...
	for (i in $data.friends) {
		...
		$stage.dialog.commFriends.append(
			$("<div>")
				...
				.append($("<div>").addClass("cfi-menu").append($("<i>").addClass("fa fa-pencil").on("click", requestEditMemo)).append($("<i>").addClass("fa fa-remove").on("click", requestRemoveFriend)))
		);
	}
			...
			$("<div>")
				...
				.append(
					$("<div>")
						...
						.html(commify(o.data.score) + " / " + commify(EXP[i - 1]) + L["PTS"])
				)
		)
		.append(
			($ex = $("<div>")
...
function replayReady() {
	...
	$data.$gp = $(".GameBox .product-title")
		...
		.append(($data.$gpc = $("<div>").addClass("game-replay-controller").append($("<button>").html(L["replayNext"]).on("click", replayNext)).append($("<button>").html(L["replayPause"]).on("click", replayPause)).append($("<button>").html(L["replayPrev"]).on("click", replayPrev))));
...
function replayStatus() {
	$data.$gpt.html($data._gpp + " (" + ($data._eventTime * 0.001).toFixed(1) + L["SECOND"] + " / " + ($data._gtt * 0.001).toFixed(1) + L["SECOND"] + ")");
}
function replayTick(stay) {
	...
	replayStatus();
	if ($data.room.events.length > $data._rf) $data._rt = addTimeout(replayTick, $data.room.events[$data._rf].time - event.time);
	else replayStop();
}
function replayStop() {
	delete $data.room;
...
function roundEnd(result, data) {
	...
	function explainReward(orgX, orgM, list) {
		...
		function row($t, h, b) {
			$t.append($("<h5>").addClass("result-me-blog-head").html(h)).append($("<h5>").addClass("result-me-blog-body").html(b));
		}
		return $R;
	}
	addTimeout(function () {
	...
	if (item.term)
		$R.append(
			$("<div>")
				...
				.html(Math.floor(item.term / 86400) + L["DATE"] + " " + L["ITEM_TERM"])
		);
	if (expire)
		$R.append(
			$("<div>")
		...
		for (i = 0; i < 3; i++) {
			addTimeout(
				function (v) {
					...
					(beat ? $stage.game.display.children(".display-text") : $stage.game.display).css("font-size", 21).animate({ "font-size": 20 }, tick);
				},
				i * tick * 2,
				i
			);
...
function pushHistory(text, mean, theme, wc) {
	...
	$stage.game.history.prepend(($v = $("<div>").addClass("ellipse history-item").width(0).animate({ width: 200 }).html(text)));
	...
	$v.append(($w = $("<div>").addClass("history-mean ellipse").append(val))).append(
		($x = $("<div>")
			.addClass("expl")
	...
	$obj
		...
		.append(
			$("<h5>")
				...
				.html((mobile ? "" : L["players"] + " ") + room.players.length + " / " + room.limit)
		)
		.append(
			$("<h5>")
...
function iImage(key, sObj) {
	...
	return obj.group.charAt(0) == "M" ? "/img/kkutu/moremi/" + obj.group.slice(1) + "/" + obj._id + gif : "/img/kkutu/shop/" + obj._id + ".png";
}
...
function renderMoremi(target, equip) {
	...
	if ((key = equip["BDG"])) {
		$obj.append($("<img>").addClass("moremies moremi-badge").attr("src", iImage(key)).css({ width: "100%", height: "100%" }));
	}
