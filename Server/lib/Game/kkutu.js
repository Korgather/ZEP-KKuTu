...
exports.Client = function (socket, profile, sid) {
	...
	*/
	...
	my.refresh = function () {
		...
		if (my.guest) {
			...
			if(Cluster.isMaster && !my.isAjae){ // null일 수는 없다.
				...
				/* Enhanced User Block System [E] */ else if (exports.NIGHT && my.isAjae === false) R.go({ result: 440 });
	...
	my.flush = function (box, equip, friends) {
		...
		DB.users
			.upsert(["_id", my.id])
			.set(!isNaN(my.money) ? ["money", my.money] : undefined, my.data && !isNaN(my.data.score) ? ["kkutu", my.data] : undefined, box ? ["box", my.box] : undefined, equip ? ["equip", my.equip] : undefined, friends ? ["friends", my.friends] : undefined)
			.on(function (__res) {
...
exports.Room = function (room, channel) {
	...
	my.drawingCanvas = function (msg, userid) {
		//msg -> Message, userid -> sender ID
		if (my.game.painter == userid) {
			// verify this data sended by painter
			let diffed = true;
